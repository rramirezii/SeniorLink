...
class LoginController extends Controller
{
    ...
    public function validateLogin(Request $request)
    {
        ...
        if ($superAdmin) {
            return response()->json(["message" => "Success", "role" => "admin", "id" => $superAdmin->id, "name" => $superAdmin->name], 200);
        } elseif ($town) {
            return response()->json(["message" => "Success", "role" => "town", "id" => $town->id, "name" => $town->name], 200);
        } elseif ($barangay) {
            return response()->json(["message" => "Success", "role" => "barangay", "id" => $barangay->id, "name" => $barangay->name], 200);
        } elseif ($establishment) {
            return response()->json(["message" => "Success", "role" => "establishment", "id" => $establishment->id, "name" => $establishment->name], 200);
        } else {
