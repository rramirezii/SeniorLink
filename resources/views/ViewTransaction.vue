<template>
      ...
      <table v-else class="table">
        ...
        <tbody>
          <tr v-for="item in filteredTableData" :key="item.id">
            ...
            <td>
              <router-link :to="{ name: 'ViewTransactions', params: { id: item.id }}">
                <button class="view-button" @click="navigateToTransaction(item.id)">View</button>
              </router-link>
            </td>
          </tr>
  ...
  export default {
    data() {
      return {
        tableHeaders: ['Date', 'Commodities/Medicine', 'Quantity', 'Amount of Purchase', 'Balance', 'Establishment Name'],  // Default headers
        tableData: [],
        searchQuery: '',
        loading: true,
    ...
    async mounted() {
      try {
        const response = await axios.get('/transactions.json');  //file should be in the `public` folder 
